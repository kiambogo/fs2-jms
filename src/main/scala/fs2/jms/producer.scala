...
package object producer {
  def textPipe[F[_]](producerSettings: JmsProducerSettings)(implicit F: ConcurrentEffect[F]): fs2.Pipe[F, String, Either[Throwable, TextMessage]] = { in =>
    ...
    Stream.eval(Queue.bounded[F, Option[ProducerSession]](producerSettings.sessionCount+1)).flatMap { queue =>
      ...
      Stream.eval_(jmsProducer.openSessions) ++ in.evalMap { elem =>
        for {
        ...
        (Stream.eval_(queue.enqueue1(None)) ++
           ...
           Stream.eval_(jmsProducer.shutdown)
        ).compile.drain
      }
    }
  }
