...
package object producer {
  def textPipe[F[_]](producerSettings: JmsProducerSettings)(implicit F: ConcurrentEffect[F]): fs2.Pipe[F, String, Either[Throwable, Message]] = {
    in =>
    ...
    def sessionCreatedCallBack(producers: Queue[F, ProducerSession]): QueueSession => Unit = { session =>
      println(s"session enqueued: $session")
      ...
      Stream.eval(Queue.bounded[F, ProducerSession](producerSettings.sessionCount)).flatMap { queue =>
        ...
        in.evalMap { elem =>
          for {
            ...
            callback <- Effect[F].async[Message] { cb =>
              val callback = MessageSentCallback(cb)
              sessionProducer.producer.send(msg, callback)
            }
